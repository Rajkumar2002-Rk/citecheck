"""Full quote-mode pass. Mirror of run_span.py; see it for the design notes."""
import json, os, sys
...
LOCK = Path("data/.quote_run.lock")
if LOCK.exists():
    pid = LOCK.read_text().strip()
    try:
        os.kill(int(pid), 0)
        sys.exit(f"a quote run is already active (pid {pid}); refusing to start a second")
    except (ProcessLookupError, ValueError):
        pass  # stale lock
LOCK.write_text(str(os.getpid()))

OUT = Path("data/quote_pass1.jsonl")
...
try:
    ...
    with OUT.open("a") as out:
        for i, f in enumerate(filings, 1):
            ...
            a = extract(client, text, mode="quote")
