...
Note what the definition doesn't say: nothing was necessarily misstated. It is
...
but you can't pin the version (JAAG, Netlist); NOT_STATED means no framework is
...
**CROSS_REFERENCED vs a real opinion.** The test isn't where the report is
...
- **Trendmaker**. the same weakness described twice, first plural then
  ...
  Two people lacking the same skill is one weakness. Answer is 1, not 2.
...
- **INKY** and **MARKY**, weaknesses given as a numbered list, 1 through 4 and
  ...
  dates its ICFR conclusion "November 31, 2022", a day that doesn't exist.
...
- **CompX International**. the disclosure-controls conclusion is the last
  ...
  disclosure controls are. Nothing is hidden; it's just buried.
...
- **MARKY**. Item 9A defines disclosure controls and then never states a
  conclusion about them. The answer isn't in the section. Worth knowing because
  the model, asked for it anyway, cited the *ICFR* conclusion instead, it
  reached for the nearest adjacent sentence when the required one wasn't there.
