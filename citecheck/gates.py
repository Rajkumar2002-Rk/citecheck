...
# --------------------------------------------------------------------------
# Gate 4 - remediated weaknesses reported as open
# --------------------------------------------------------------------------

# The hand-labeled ground truth exposed exactly one systematic claim error: the
# model reports material weaknesses that the filing says were already fixed. A
# remediated weakness is a company reporting it solved a problem; an open one
# means ICFR is ineffective and the auditor issues an adverse opinion. Close to
# opposite facts, often two sentences apart.
#
# No citation check catches this, because the citation is correct -- the model
# quotes the remediation paragraph accurately and counts it anyway.

# Completed remediation. Every alternative is taken from filings in this corpus.
_REMEDIATION_DONE = re.compile(
    r"\b(?:was|were|been|be)\s+remediated\b"
    r"|\bremediated\s+as\s+of\b"
    r"|\bable\s+to\s+remediate\b"
    r"|\bno\s+longer\s+exists?\b"
    r"|\bhas\s+been\s+remediated\b",
    re.IGNORECASE,
)

# Phrases that contain the same words but mean the opposite. Checked first.
# "we were unable to remediate this material weakness as of December 31, 2023"
# and "will not be considered fully remediated until the remediation actions
# are tested" both match the pattern above and both mean still open.
_REMEDIATION_NEGATED = re.compile(
    r"\bunable\s+to\s+remediate\b"
    r"|\bnot\s+(?:be\s+)?(?:considered\s+)?(?:fully\s+)?remediated\b"
    r"|\bhad\s+not\s+been\s+remediated\b"
    r"|\bwill\s+(?:not\s+)?(?:be\s+able\s+to\s+)?remediate\b"
    r"|\buntil\b[^.]{0,120}\bremediat\w+"
    r"|\bcontinues?\s+to\s+exist\b"
    r"|\bremains?\b[^.]{0,60}\b(?:open|unremediated)\b",
    re.IGNORECASE,
)

REMEDIATION_WINDOW = 700


def gate_remediation(record, text: str, resolved: dict[str, tuple[int, int]]) -> GateResult:
    """Flag a reported material weakness whose own citation says it was fixed."""
    result = GateResult(resolved=dict(resolved))

    for index, weakness in enumerate(record.material_weaknesses):
        if weakness.remediation_status.value is RemediationStatus.REMEDIATED:
            continue  # the model already says it is closed; counting is the caller's job

        path = f"material_weaknesses[{index}].description"
        span = resolved.get(path)
        if span is None:
            continue
        context = text[max(0, span[0] - REMEDIATION_WINDOW) : span[1] + REMEDIATION_WINDOW]

        # Negation wins: the same verbs appear in both, so a sentence saying the
        # company could NOT remediate must never be read as completion.
        if _REMEDIATION_NEGATED.search(context):
            continue
        if _REMEDIATION_DONE.search(context):
            result.findings.append(Finding(
                "remediation", path, Defect.CONSISTENCY,
                "reported as an open material weakness, but the surrounding text "
                "states it was remediated; a remediated weakness does not count "
                "toward the year-end total", span))
    return result
